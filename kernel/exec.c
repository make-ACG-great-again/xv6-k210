...
int exec(char *path, char **argv)
{
  ...
  return argc + 1; // this ends up in a0, the first argument to main(argc, argv)
...
int execve(char *path, char **argv, char** envp)
{
  char *s, *last;
  int i, off;
  uint64 argc, envc, sz = 0, sp, ustack[MAXARG+1], envstack[MAXARG+1], stackbase;
  struct elfhdr elf;
  struct dirent *ep;
  struct proghdr ph;
  pagetable_t pagetable = 0, oldpagetable;
  ...
  // Push environment variables, prepare rest of stack in envstack.
  ...
  if(sp < stackbase)
    goto bad;
  if(copyout(pagetable, sp, (char *)envstack, (envc+1)*sizeof(uint64)) < 0)
    goto bad;
